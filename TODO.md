...
- [ ] (menu-owner-expansion) Run `OwnerExpansion` on the menu-bar search path too — `from:me` silently means something else there. `MCPHandlers.searchEmails` (`MCPHandlers:69-74`) rewrites `from:me`/`to:me`/`cc:me`/`in:sent` against `IndexDB.ownerIdentities()` before compiling, but `StatusItemController.compiledSource` (`StatusItemController:219-224`) goes straight from `QueryParser.parse` to `Evaluator.compile`. So in the menu's search box `from:me` compiles to an FTS prefix match `me*` on the sender column and quietly matches senders like `melissa@…`; `in:sent` still works but only as the canonical Sent mailbox, missing Gmail's flat-label sent mail. Same DSL string, two meanings, no error either way. Fix: hoist the `referencesOwner` → `ownerIdentities` → `rewrite` block into a shared helper both call sites use (the guard already keeps the extra query off the common path). Note `compiledSource` is sync and `ownerIdentities` is an actor call, so the menu path needs the compile step moved into the existing `refreshTask`. Cost: S.
